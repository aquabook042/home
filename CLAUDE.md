...
**Issue タイトル**: `[C{連番4桁}]{戦術名}_{Rule Title}`
- 戦術名は Sigma ルールの主要タグから判断する（例: `Persistence`, `Discovery`, `PrivilegeEscalation`, `LateralMovement` 等）
- 連番はプロジェクト全体の Sigma/Signal Issue を通じた通し番号（4桁ゼロ埋め）
- 例: `[C0001]Persistence_New Domain User Account Created`
